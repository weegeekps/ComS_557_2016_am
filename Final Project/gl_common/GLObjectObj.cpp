...
{
	_file_ok = false;
	_file_ok = load_obj(filename.c_str(), _vertices, _textures, _normals, _elements);
}
...
bool GLObjectObj::load_obj(const char* filename, vector<glm::vec3>& vertices, vector<glm::vec2>& textures, vector<glm::vec3>& normals, vector<GLuint>& elements)
{
...
void GLObjectObj::initVBO(void)
{
	...
	int locTexture = glGetAttribLocation(_program, "in_TexCoord");
	glBindBuffer(GL_ARRAY_BUFFER, _vboID[2]);
	glBufferData(GL_ARRAY_BUFFER, _textures.size() * 2 * sizeof(GLfloat), &textures[0], GL_STATIC_DRAW);

	glVertexAttribPointer((GLuint)locTexture, 2, GL_FLOAT, GL_FALSE, 0, 0);
